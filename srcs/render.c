...
/*   Updated: 2019/12/22 08:28:50 by pduhard-    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
int		ray_intersect(t_3vecf orig, t_3vecf dir, float *dist, t_sphere *sphere)
{
	...
	l = sub_3vecf(assign_3vecf(sphere->x, sphere->y, sphere->z), orig);
...
int		cast_ray(t_3vecf orig, t_3vecf dir, t_data *data)// obj number ..
{
	...
	if ((hit_object = ray_trace(orig, dir, objects, &dist)))
	{
	//	return (0xffffff);
		t_3vecf		hit_point;
		...
		normal = sub_3vecf(hit_point, assign_3vecf(param->x, param->y, param->z));
...
//			light_intensity = assign_3vecf(1, 1, 1);
			...
			t_obj	*hit_obj_l = ray_trace(m_light_orig, m_light_dir, objects, &l_dist); 
			if (!hit_obj_l)
			{
				float	d_pd_m_l_dir = dot_product_3vecf(normal, m_light_dir);
				if (d_pd_m_l_dir > 0.f)
				{
					diffuse.val[0] += 0.18 * light_intensity.val[0];
					diffuse.val[1] += 0.18 * light_intensity.val[1];
					diffuse.val[2] += 0.18 * light_intensity.val[2];
				...
				{
					specular.val[0] += light_intensity.val[0] * powf(d_pd_r_m_l_dir, 10.0);
					specular.val[1] += light_intensity.val[1] * powf(d_pd_r_m_l_dir, 10.0);
					specular.val[2] += light_intensity.val[2] * powf(d_pd_r_m_l_dir, 10.0);
				}
	...
	//	print_vec(specular.val);
		float clr_tmp = (diffuse.val[0] * 0.8 + specular.val[0] * 0.2) * (float)((hit_object->color >> 16) & 0xff);
		hit_color = 0;
		hit_color |= (int)clr_tmp << 16;
		clr_tmp = (diffuse.val[1] * 0.8 + specular.val[1] * 0.2) * (float)((hit_object->color >> 8) & 0xff);
		hit_color |= (int)clr_tmp << 8;
		clr_tmp = (diffuse.val[2] * 0.8 + specular.val[2] * 0.2) * (float)(hit_object->color & 0xff);
